...
const Dashboard = ({ userData, onLogout }) => {
  ...
  // ADD THE NEW SALARY MASKING FUNCTIONS HERE:
  const maskCurrency = (amount) => {
  ...
  const togglePayrollVisibility = () => {
    setPayrollVisible(!payrollVisible);
  };
  useEffect(() => {
    const initialExpandedState = {};
    timesheetData.forEach(day => {
      initialExpandedState[day.date] = true;
    });
  ...
  const fetchPayrollDetail = async (periodId) => {
    try {
      ...
      const transformedData = {
        ...
        tax_deductions: apiData.calculated_salary * 0.1,
        benefits_deductions: 0,
        other_deductions: apiData.undertime_deduction || 0,
        net_pay: apiData.calculated_salary * 0.9,
                          ...
                          <div className="space-y-4">
                            {employeeTimesheetData.map((day) => (
                              <TimesheetDay
                                key={day.date}
                                day={day}
                                isExpanded={expandedEmployeeDays[day.date] !== false}
                  ...
                  {dashboardData?.current_salary && (
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-4">
                      <div className="flex items-center mb-3">
                        <FontAwesomeIcon icon={faMoneyBillWave} className="text-green-500 w-5 h-5 mr-2" />
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-lg font-semibold text-gray-900 dark:text-white">
                              {salaryVisible
                                ...
                                : maskCurrency(dashboardData.current_salary.monthly_salary)
                              }
                            </div>
                          </div>
                          <button
                            onClick={toggleSalaryVisibility}
                            className="ml-3 p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
                            title={salaryVisible ? "Hide salary" : "Show salary"}
                          >
                            <FontAwesomeIcon icon={salaryVisible ? faEyeSlash : faEye} className="w-4 h-4" />
                          </button>
                        </div>
                        ...
                        <div>
                          <div className="text-sm text-gray-600 dark:text-gray-400">Effective From</div>
                          <div className="text-lg font-semibold text-gray-900 dark:text-white">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 border border-gray-100 dark:border-gray-700">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4">
                          ...
                          <div className="flex items-center">
                            <span className={`inline-block h-3 w-3 rounded-full mr-2 ${dashboardData?.is_working ? 'bg-green-500 animate-pulse' : 'bg-gray-400'
                              }`}></span>
                            ...
                            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                              {payrollRecords.map((record) => (
                                <tr key={record.id} className="border-b border-gray-200 dark:border-gray-700">
                                  <td className="px-2 sm:px-4 py-3 text-gray-700 dark:text-gray-300 text-xs sm:text-sm">
                                    {record.pay_period}
                                  </td>
                                  <td className="px-2 sm:px-4 py-3 text-gray-700 dark:text-gray-300 text-xs sm:text-sm">
                                    <div className="space-y-1">
                                      <div className="text-xs">Regular: {record.regular_hours}h</div>
